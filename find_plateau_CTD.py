...
def new_segment(segments, seg_num, slope):
    ...
    '''
    segments[seg_num] = {'bottle': isclose(slope, 0, abs_tol= atol), "depth": math.nan, "usable": False, "points" : []}
    return segments
...
casts = cast_dic[1] # which cast
ctd = '/Volumes/GeringSSD/GU201905_CTD/ctd001.evl' # which CTD
outfile_path = '/Volumes/GeringSSD/segments_cdt001.json'
...
for j in range(len(x)-1):
    ...
    if not math.isclose(new_slope, current_slope, abs_tol=atol):
        if segments[seg_num]["bottle"] == isclose(new_slope, 0, abs_tol= atol):
