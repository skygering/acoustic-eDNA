...
abc_edna_scatter <- function(df){
  ...
  # Need a data frame with both ABC and X.12S.final columns
  p <- ggplot(df, aes(X.12S.rank, ABC, color = factor(X.12S.rank))) +
    geom_point() + theme_bw() + 
    theme(legend.position = "none", plot.title = element_text(size = 22, face = "bold", hjust = 0.5), 
          axis.title = element_text(size = 18),
          axis.text.x = element_text(size = 16),
    ...
    xlab("12S PCR eDNA Rank") + ylab("ABC for Fish Scattering") + 
    scale_x_continuous(labels= c("No", "EL", "L", "M", "H"))
  
  return (p)
}

read_eDNA <- function(edna_data, transducer_depth = 5, filtration_vol = 2){
  ...
  # Remove columns that will no longer be used and rename depth column to eDNA.Depth
