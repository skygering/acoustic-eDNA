from scipy.signal.filter_design import _norm_factor
...
def subset_segments_Sv(segment_dic, raw_files, toffsets, doffsets, transducer_offset, frequencies = [18000, 38000, 120000, 200000], json_format = False):
    '''
    ...
    Inputs: segment_dic (segment dictionary) - segment dictionary after running create_segments_dic or interactive_segment_maker
            ...
            json_format (boolean) - if True output will be writable to json file and will have Sv data points in an array - not Sv object
    ...
    for num in segment_dic:
        ...
        if seg["usable"]:
            ...
            fq_dic = {}
            subset_segment_dic[round(y_mean)] = fq_dic
            ...
            for fq in frequencies:
                Sv, cal =  raw_to_Sv(ek80, fq, transducer_offset)
                if cal.sample_interval != cal18.sample_interval:
                    Sv.match_samples(Sv18)
                subset_Sv = crop_Sv(Sv.copy(), (x_min_idx, x_max_idx), (y_min_idx, y_max_idx)) # see if processed_data copy is better
                if json_format:
                    fq_dic[fq] = subset_Sv.data.tolist() # numpy arrays are not exportable to json goes to a nested list
                else:
                    fq_dic[fq] = subset_Sv # data is still a numpy array
